...
void EnemyMissile::Initialize(Model* model, const Vector3& position, Player* target) {
	assert(model);
	model_ = model;
	target_ = target;
...
void EnemyMissile::Update() {
	...
	if (target_ && !target_->IsDead()) {
		...
		if (dot < 0.0f) {
			...
			worldTransform_.rotation_.y = std::atan2(velocity_.x, velocity_.z) + static_cast<float>(M_PI);
...
void EnemyMissile::Draw(const Camera& camera) { model_->Draw(worldTransform_, camera); }
