...
void Player::Update() {
	...
	if (onGround_) {
		...
		// ジャンプ入力 (キーをスペースに、押した瞬間だけ反応するように変更)
		...
		// ▼ 空中状態の処理
		velocity_.y -= kGravityAcceleration; // 重力
		// (修正) 空中では左右の加速をなくし、空気抵抗で少し減速させる
		velocity_.x *= (1.0f - kAttenuation / 4.0f);
	}
	// 速度制限
	...
	// (追加) 空中にいるときにZ軸回転させる
	if (!onGround_) {
		const float kRotationSpeed = 0.2f;
